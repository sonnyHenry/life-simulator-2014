...
### M5 第二十九轮(去掉高考地点 + 玩家性别选择 + 恋爱线性别适配)

由 Claude Code(Opus 4.8)完成,内容版本 0.18.0 → 0.19.0。玩家反馈三连:去掉开局「你在哪里参加高考?」、开局改选男/女、后续事件(主要恋爱线)按性别调整称呼。本轮与第二十八轮(补职业线)在两个会话里并行推进,合并入库时代码自动合干净、仅 `AGENT_HANDOFF.md` 文档冲突,合并后重跑门禁全绿(见下方验证)。
...
### M5 第三十一轮(特质体感强化:专属事件 + UI 标签)

由 Claude Code(Fable 5)完成,内容版本 0.20.0 → 0.21.0,事件 147 → 153。起因是用户玩测第三十轮后反馈:"除了开局有 2 个默认性格以外,跟之前没什么区别"——诊断结论:上一轮的两套机制统计上生效但**单局不可感知**(特质专属选项混在普通选项里无标识;导演器改变抽取概率,单局无对照即隐形)。本轮把系统做成"看得见":

- **特质 UI 标签(纯 view 层,内容零改动)**:`engine.ts` 新增 `requiredTraitLabel(cond)`——识别选项 `visibleIf` / 事件 `trigger` 里"必然要求某特质"的条件(顶层 `{flag:'trait_*'}` 或 `all` 分支内,`any` 里的备选不算),命中则给文案加 `【特质名】` 前缀。所有特质门控的选项和事件在三端 UI 上自动显示「【社牛】不用竞选词了…」式标签,玩家每次都能看见"这是我的特质带来的"。单测 +1(25)。
- **6 个特质专属事件**(新建 `events/trait-moments.ts`,每特质 1 个,`mandatory` + 年份窗口 + trait flag 门控,**带该特质的玩家该年必触发**;语境全人群通用,在读/读研/上班都成立,不挂 working):胆大 2018《同学群里的暴富神话》(币圈神话,冲/看戏/研究明白三选)、卷王 2019《凌晨四点的朋友圈》("图什么呢"的灵魂拷问)、社牛 2020《隔着屏幕的热闹》(疫情断социal电,线上局/学独处)、松弛 2021《「躺平」上了热搜》(自己的生活方式成了社会议题)、恋家 2022《改签了三次的车票》(回家难,折腾回去/日子过成天天在家)、细腻 2024《停更的朋友圈》(注意到别人注意不到的沉默)。每局抽 2 特质 → **必然遇到 2 个别人这局没有的专属事件**(300 局专项验证人均恰好 2.00)。
...
由 Codex 完成。新增 `pnpm repetition -n 300 --seed 42`,复用 `simulate` 的确定性 bot,统计相邻两局 Jaccard 重合率、去强制节点重合率、连续三局独有比例、来源拆分、高频事件、再见间隔、配置差异和 `variantGroup` 数量。工具只做诊断,不改变游戏抽取与 seed 复现。`simulate.ts` 的 `runOne`/类型改为可复用导出,直接执行 CLI 时行为不变。

首轮 300 局基线:平均每局 40.0 个事件;相邻重合 31.3%;去强制节点后 25.0%;连续三局独有比例 63.8%。按来源相邻重合率为强制节点 45.6%、NPC 线 32.8%、职业线 17.0%、公共池 22.0%、主时间线 26.7%。100% 出现的节点包括 2014 红包大战、2016 校园新生意、初恋操场心动和毕业散伙饭,说明下一轮应优先给固定时间线节点补变体,而不是扩普通公共池。

### M5 第四十一轮(首批固定节点上下文变体)

由 Codex 完成,内容版本 0.28.0 → 0.29.0,事件 175 → 181。利用现有 `variantGroup` 给三个原本 100% 单一出现的节点各做 3 个版本:2014 移动浪潮读取家境(农村预算/个体户线上转型/普通红包大战);2016 校园新生意读取室友 2015 的合伙/旁观选择(共同账本与债务/庆功群外的直播邀请/普通校园贷风口);2018 散伙饭读取共同创业与 `dorm_bond`(账本收尾/最后一把游戏/普通告别)。所有分支 trigger 互斥,开场、选项和结果均实质不同。

固定 seed 300 局对照:整体相邻重合率 31.3% → 28.6%;强制节点 45.6% → 37.8%;连续三局独有比例 63.8% → 65.3%。去强制节点指标 25.0% → 25.6%,符合本轮只改强制节点的预期。验证:typecheck / test 30 / validate 0err0warn / simulate 10000 全过(181/181、26 结局全可达、最大结局 23.1%、兜底 2.1%、提前结局 5.1%)。

### M5 第四十二轮(第二批固定节点上下文变体)

由 Codex 完成,内容版本 0.29.0 → 0.30.0,事件 181 → 187。2022 解封按疫情志愿者/恋家特质/普通经历分流,回收红马甲、交棒与无需报备的回家车票;2024 追风口按科技金融/公共服务职业/本地通用线分流,让风口分别成为岗位重排、工作工具或朋友圈诱惑;2025 三十岁回望按有房/裁员重启/普通人生分流,把钥匙账单与履历断行前置到玩家决策前。2023 特质成长保留单事件,因为其 12 条路线本已按两项特质动态展示,不为优化 eventId 指标复制逻辑。

固定 seed 300 局对照:整体相邻重合率 28.6% → 25.2%;强制节点 37.8% → 28.9%;连续三局独有比例 65.3% → 67.6%。验证:typecheck / test 30 / validate 0err0warn / simulate 10000 全过(187/187、26 结局全可达、最大结局 23.0%、兜底 1.7%、提前结局 5.1%)。

### M5 第四十三轮(必然恋爱线的条件化相识)

由 Codex 完成,内容版本 0.30.0 → 0.31.0。core `GameEvent` 新增 `presentationVariants`,事件展示时取首个条件命中的 title/text,但保持 eventId、choices、outcomes 与 NPC stage 不变。初恋入口据专业/特质显示四种相识:科技金融课程项目、教育医学心理见习、社牛校园活动、默认社团操场。这样恋爱线仍必然出现且完整汇入原 `together/missed` 后续,不会为降低统计 ID 人为复制状态机。重复率工具会给高频事件标注“条件开场N版”,区分技术 ID 重复与玩家实际文案重复。新增核心单测验证条件开场不改变 eventId。验证:typecheck / test 31 / validate 0err0warn / simulate 10000 全过(187/187、26 结局全可达,分布与上一轮完全一致);repetition 冒烟确认初恋显示“条件开场4版”、特质成长显示“内部12条成长路线”。

### M5 第四十四轮(高频时代节点的条件视角)

由 Codex 完成,内容版本 0.31.0 → 0.32.0。2015 牛市按家境显示四种入场压力;2019 创业/裁员按科技金融、公共服务、本地与默认职业显示四种观察位置;2020 暂停键按恋家、科技金融、体制教育与默认返乡显示四种开场,保留原疫情三年 schedule。validate 补齐 `presentationVariants` 的 trait/fn/npc/恒假条件检查,并新增每个事件 choice id 唯一性门禁;门禁随即发现并修复旧事件 `ev_random_wedding_invite` 两个 choice 同为 `c`、导致后一个可见选项点击后执行前一个动作的问题。验证:typecheck / test 31 / validate 0err0warn / simulate 10000 全过(187/187、26 结局全可达、最大结局23.0%、兜底1.7%、提前结局5.1%),婚礼选项修复只造成统计舍入级变化。

### M5 第四十五轮(Hades 式小回响)

由 Codex 完成,内容版本 0.32.0 → 0.33.0。core `GameEvent` 新增 `contextLines`,事件展示时在正文后、选项前追加首个条件命中的短回响,不改变 eventId/choices/effects。首批接入三个高频通用事件:领导反馈回收导师红圈方案、2024转岗能力或效率表;相亲局回收未寄日记、操场拒绝或沉默视频;家里电话回收解封热饭、房贷或发小线的故乡疏离。validate 同步覆盖 context line 的 trait/fn/npc/恒假条件,核心展示测试同步断言开场与回响拼接。验证:typecheck / test 31 / validate 0err0warn / simulate 10000 全过(187/187、26结局全可达,分布与0.32.0完全一致)。

### M5 第四十六轮(高频小回响覆盖)

由 Codex 完成,内容版本 0.33.0 → 0.34.0。重复率仪表盘新增“高频事件感知变体覆盖”:对出现率≥50%的事件统计 `variantGroup`、`presentationVariants`、`contextLines` 或内部成长路线覆盖,并直接列出待补事件。按 seed 42–341 的榜单为《换季重感冒》《绿码时代》《换手机》《老同学的朋友圈》各补 3 条首匹配回响,分别回收疫情选择、2014 家庭移动生意、P2P 得失与大学毕业记忆。该指标衡量玩家看到的文案变化,不冒充 eventId 重合率下降。

### M5 第四十七轮(高频感知变体全覆盖)

由 Codex 完成,内容版本 0.34.0 → 0.35.0。为仪表盘仅余的两个出现率≥50%裸事件各补 3 条 `contextLines`:《朋友圈的红点》回收未寄日记、异地沉默和操场拒绝;《老同学的“项目”》回收室友借债/拒债和 2015 股市第一课。固定 seed 300 局的高频事件感知变体覆盖由 16/18(88.9%)升至18/18(100%)。注意这是内容能力覆盖,具体一局仍需对应 flag 命中才显示回响。

### M5 第四十八轮(回响实际命中率仪表盘)

由 Codex 完成,内容版本保持 0.35.0。`simulate.runOne` 新增只读 telemetry:EVENT 展示时按 `engine.view` 相同的 `rngState` 与条件求值顺序记录实际命中的条件开场和 context line 索引;不改 GameState、RNG 流或抽取。`repetition` 新增平均每局回响、至少命中一条的对局比例、带回响事件出现后的实际命中率、逐事件命中率、零命中和低命中单句。seed 42–341 基线:平均 2.90 条/局,97.0% 对局至少一条,总体命中 869/1654(52.5%),条件开场 2.26 个/局,27 条回响无零命中。薄弱项为重感冒5.2%、家里电话13.3%、老同学朋友圈11.4%、换手机15.8%;后续应补真实可达的前史连接,目标 4–6 条/局。

### M5 第四十九轮(回响密度达到目标下沿)

由 Codex 完成,内容版本 0.35.0 → 0.36.0。新增15条真实前史连接:2023性格成长6条(P2P、2015股市课、疫情三种行动),2020基金上车3条(股市/P2P),家里电话补居家办公,老同学朋友圈补普通宿舍羁绊,换手机补股市课。固定 seed 300局平均实际回响2.90→4.23条,至少一条对局97.0%→98.3%,带回响事件出现后命中52.5%→61.5%;其中性格成长86.1%、基金上车63.8%、换手机55.9%。没有添加无条件 context line,所有新增句子均依赖已发生 flag。
...
pnpm repetition -n 300           # 跨局重复率基线;固定 seed 便于前后对照
...
第六十六轮由 Codex 完成关系反馈降噪。所有 warm/cool `outcomeTag` 与 historyCount 机制保持不变;`relationshipHint` 改为每条 NPC 线首次记录时只教学一次,之后普通打点静默,仅在该线 warm 达到专属收束门槛时再给一次里程碑反馈。核心测试覆盖首次提示、普通静默、恰好到阈值和超过阈值不重复四种情况。
