...
```text
事件 108, 结局 19, NPC 5, 题目 37, 收入规则 19
...
```text
事件覆盖: 107/108 (未触发: ev_drama_stock_options)
金钱分位: p10=¥115700 p50=¥219400 p90=¥404701
心态分位: p10=5 p50=32 p90=72
提前结局占比: 6.6%
```

**已知边界情况**:`ev_drama_stock_options`(门控 `career_cs + big_platform_start + year 2020-2021`,M5 第二十二轮前就存在)命中率本身就低,`baseSeed=42` 在 n=1000 时偶发漏抽,与本轮改动无关。`pnpm simulate -n 3000 --check` 稳定 108/108 全覆盖、`✅ 分布目标校验通过(全覆盖、全可达、无结局>40%、兜底≤35%、提前结局≤10%)`,本轮 4 个 Phase 节点都复核过这一点。

按职业线分组(n=1000,随机 bot):金融均财 ¥377,760/心态均值 26(样本仅 11 局)、医学均财 ¥238,004/心态均值 53(56 局)、计算机均财 ¥356,977/心态均值 21、教育均财 ¥260,026/心态均值 45(师范线收入分档重构 + 双重计费 bug 修复后)、体制内均财 ¥175,959/心态均值 54。
...
- **`GAME_DESIGN.md` 第四节"职业线(4 条精做)"密度**:计算机线现有 10 个专属事件(2018-2026 全程覆盖)、师范线 8 个(2018-2026 全程覆盖),金融/医学仍是 5/4 个且只覆盖到 2021/2023——本轮只补了计算机/师范的断更(用户已确认本轮范围),金融/医学线 2024-2026 若也要做"回望"收官事件,是明确留给下一轮的候选项。内容总量 108 个,仍低于设计文档最初设想的约 125 个。
